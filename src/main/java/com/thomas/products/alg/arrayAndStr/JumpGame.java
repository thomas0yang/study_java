...
//        assert exceptedResult1 == canJump(num1);
        assert exceptedResult1 == canJump2(num1);
        assert exceptedResult1 == canJump3(num1);
        int[] num2 = {3, 2, 1, 0, 4};
        boolean exceptedResult2 = false;
//        assert exceptedResult2 == canJump(num2);
        assert exceptedResult2 == canJump2(num2);
        assert exceptedResult2 == canJump3(num2);
    }
    ...
    private static boolean canJump2(int[] nums) {
        ...
        for (int i = 0; i < nums.length - 1; i++) {
            if (i <= maxIndex) { //当前位置小于最大索引位置，在可跳跃能力范围内判断
