...
public final class DateUtil {
    ...
    public static String timeToString(final Date date) {
        return dateTimeToString(date, "H:mm:ss");
    }
    ...
    public static Date getMonthORDaysAgo(int month, int days) {
        return getMonthORDaysAgo(new Date(), month, days);
    }

    public static Date getMonthORDaysAgo(Date date, int month, int days) {
        GregorianCalendar canlendar = new GregorianCalendar();
        canlendar.setTime(date);
        if (month != 0) {
            canlendar.add(GregorianCalendar.MONTH, month);
        }
        if (days != 0) {
            canlendar.add(GregorianCalendar.DAY_OF_MONTH, days);
        }
        canlendar.set(canlendar.HOUR_OF_DAY, 0);
        canlendar.set(canlendar.MINUTE, 0);
        canlendar.set(canlendar.SECOND, 0);
        canlendar.set(canlendar.MILLISECOND, 0);
        return canlendar.getTime();
    }

    public static String getMonthORDaysAgo(int month, int days, String sformat) {
        Date date = getMonthORDaysAgo(month, days);
    ...
    // 获取上几个自然周日期列表，周一到周日
    public static LinkedList<Date> getLastWeekDays(Date date, int weekcount) {
        ...
        int dayofweek = canlendar.get(GregorianCalendar.DAY_OF_WEEK);
        if (dayofweek == 1) {
            dayofweek = 8;
        }
        canlendar.add(GregorianCalendar.DAY_OF_MONTH,
                ((dayofweek + 5) + 7 * (weekcount - 1)) * -1);
        for (int i = 0; i < 7 * weekcount; i++) {
            list.add(canlendar.getTime());
            canlendar.add(GregorianCalendar.DAY_OF_MONTH, 1);
        }
        return list;
    }

    public static String joinDays(LinkedList days, String format) {
        StringBuffer result = new StringBuffer("{");
        for (int i = 0; i < days.size(); i++) {
            result.append(DateUtil.formatDate((Date) days.get(i), format));
            result.append(",");
        }
    ...
    public static LinkedList getConsecutiveDays(String sstarttime,
            String sendtime, String format) {
        LinkedList result = new LinkedList();
        Date starttime = DateUtil.parserDateStr(sstarttime, format);
        Date endtime = DateUtil.parserDateStr(sendtime, format);

        GregorianCalendar canlendar = new GregorianCalendar();
        canlendar.setTime(starttime);

        while (!canlendar.getTime().after(endtime)) {
            result.add(canlendar.getTime());
            canlendar.add(GregorianCalendar.DAY_OF_MONTH, 1);
        }
        return result;
    }
